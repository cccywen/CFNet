...
class CAM_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.bmm(attention, proj_value).view(batch_size, chnnels, height, width)
...
class PoolUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.up1(torch.add(c4, fa4), self.ca3(c3))
        p1 = torch.add(x, Upsample(fa3, csize[2]))
        x = self.up2(p1, self.ca2(c2))
        p2 = torch.add(x, Upsample(fa2, csize[1]))
        x = self.up3(p2, self.ca1(c1))
