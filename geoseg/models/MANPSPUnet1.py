...
class PSPModule(nn.Module):
    def __init__(self, in_channels, bin_sizes):
        ...
        self.conv1 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding='same')
    ...
    def forward(self, features):
        ...
        pyramids[1] = self.conv1(F.interpolate(pyramids[1], size=(2, 2), mode='bilinear', align_corners=True))
        pyramids[2] = self.conv2(F.interpolate(pyramids[2] + pyramids[1], size=(3, 3), mode='bilinear', align_corners=True))
        pyramids[3] = self.conv3(F.interpolate(pyramids[3] + pyramids[2], size=(6, 6), mode='bilinear', align_corners=True))
        pyramids[4] = self.conv4(F.interpolate(pyramids[4] + pyramids[3], size=(h, w), mode='bilinear', align_corners=True))
