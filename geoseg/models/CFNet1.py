...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, size,bilinear=True):
        ...
        self.attention = CNNTransformer_record(dim=out_channels, depth=6, heads=6, dim_head=64, dropout=0.1, num_patches=size)
...
class PoolUNet(nn.Module):
    def __init__(self, n_classes, backbone='resnet18', bilinear=False):
        ...
        self.transformer = CNNTransformer_record(dim=512, depth=12, heads=8, dim_head=64, dropout=0.1, num_patches=1024)

        self.up1 = Up(512, 256, 1024, bilinear)
        self.uni1 = nn.Conv2d(256, n_classes, kernel_size=1, bias=False)
        self.up2 = Up1(256, 128, 4096, bilinear)
        self.uni2 = nn.Conv2d(128, n_classes, kernel_size=1, bias=False)
        self.up3 = Up1(128, 64, 512, bilinear)
