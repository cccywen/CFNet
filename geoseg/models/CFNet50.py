...
class PoolUNet(nn.Module):
    def __init__(self, n_classes, backbone='resnet50', bilinear=False):
        ...
        self.attention1 = nn.Sequential(PAM_Module(m_out_sz), nn.Conv2d(m_out_sz, 256, kernel_size=1))
        self.attention2 = nn.Sequential(PAM_Module(m_out_sz), nn.Conv2d(m_out_sz, 512, kernel_size=1))
        self.attention3 = nn.Sequential(PAM_Module(m_out_sz), nn.Conv2d(m_out_sz, 1024, kernel_size=1))
        self.attention4 = nn.Sequential(PAM_Module(m_out_sz), nn.Conv2d(m_out_sz, 2048, kernel_size=1))

        self.transformer = CNNTransformer_record(dim=2048, depth=12, heads=8, dim_head=64, dropout=0.1, num_patches=256)
        ...
        self.up4 = nn.ConvTranspose2d(256, 32, kernel_size=2, stride=2)
        # 降维太快了
